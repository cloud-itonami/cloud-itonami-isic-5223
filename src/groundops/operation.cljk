(ns groundops.operation
  "OperationActor -- one ground-handling-coordination request = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (GroundOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Airport Ground Operations
  ...
  One graph run = one ground-handling-coordination request (intake ->
  ...
  decision to a human ground-operations coordinator. The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:flag-ramp-safety-concern` ALWAYS reaches this node when the
  governor is clean (never auto, at any phase) -- see
  `groundops.phase`. `:schedule-ground-operation`/`:coordinate-
  equipment-maintenance` also always reach it when clean, at every
  phase this R0 enables."
...
(defn build
  ...
  `groundops.store/Store`).
  opts:
    :advisor      -- a `groundops.groundopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (groundopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; GroundOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (groundopsllm/-advise advisor store request)]
            {:proposal p :audit [(groundopsllm/trace request p)]})))

      ;; Airport Ground Operations Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :ramp-safety-flag
                                          :else :low-confidence))
                        :phase ph
